...
export async function POST(request: NextRequest) {
	...
	if (type === "content") {
		...
		await writeFile(process.cwd() + `/public/preview/${picture.name}`, Buffer.from(await picture.arrayBuffer()))
		await writeFile(process.cwd() + `/public/preview/${poster.name}`, Buffer.from(await poster.arrayBuffer()))
		await writeFile(process.cwd() + `/public/preview/${postertext.name}`, Buffer.from(await postertext.arrayBuffer()))
		...
		await writeFile(process.cwd() + `/public/news/${file.name}`, Buffer.from(await file.arrayBuffer()))
